    ...
    return new Promise(async resolve => {
        ...
        if (['.jpg', '.jpeg', '.png', '.webp', '.gif'].includes(ext)) {
            fs.unlink(`./storage/file_storage/${postData.file}`, async (err) => {
                if (err) {
                    sysController.log(`e/Ошибка удаления файла [deletePost|image]: ${err}`)
                    resolve({ rslt: 'e', msg: `e/Ошибка удаления файла [deletePost|image]: ${err}` })
                    return
                } else {
            ...
            fs.unlink(`./storage/file_storage/${postData.file}`, async (err) => {
                if (err) {
                    sysController.log(`e/Ошибка удаления файла [deletePost|video]: ${err}`)
                    resolve({ rslt: 'e', msg: `e/Ошибка удаления файла [deletePost|video]: ${err}` })
                    return
                } else {
                    fs.unlink(`./storage/video_thumbnails/THUMBFOR-${path.parse(postData.file).name}.jpg`, async (err) => {
                        if (err) {
                            sysController.log(`e/Ошибка удаления файла [deletePost|video|thumbnail]: ${err}`)
                            resolve({ rslt: 'e', msg: `e/Ошибка удаления файла [deletePost|video|thumbnail]: ${err}` })
                        } else {
                            const result = await sysController.dbinteract.deletePost(request.post)
                            resolve(result)
                        }
